            ...
            <form action="{{ route('admin.fornecedores.update', $fornecedor->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="contacto" name="contacto" 
                           ...
                           value="{{ old('contacto', $fornecedor->contacto) }}" required>
                    @error('contacto')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="endereco" name="endereco" 
                           ...
                           value="{{ old('endereco', $fornecedor->endereco) }}" required>
                    @error('endereco')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select id="tipo_parceria" name="tipo_parceria" 
                        ...
                        <option value="FORNECEDOR" {{ old('tipo_parceria', $fornecedor->tipo_parceria) == 'Fornecimento' ? 'selected' : '' }}>Fornecedor</option>
                        <option value="PARCEIRO" {{ old('tipo_parceria', $fornecedor->tipo_parceria) == 'Parceria' ? 'selected' : '' }}>Parceria</option>
                    </select>
                    @error('tipo_parceria')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea id="observacoes" name="observacoes" 
                              ...
                              rows="3">{{ old('observacoes', $fornecedor->observacoes) }}</textarea>
                    @error('observacoes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <select id="tipos_stock" name="tipos_stock[]" 
                        ...
                        @foreach($tiposStock as $tipo)
                            <option value="{{ $tipo->id }}" 
                                {{ in_array($tipo->id, $selectedTipos) ? 'selected' : '' }}>
                                {{ $tipo->nome }}
                            </option>
                        @endforeach
                    </select>
